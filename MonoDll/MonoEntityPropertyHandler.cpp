...
extern "C"
{
	_declspec(dllexport) void __cdecl _SetProperty(EntityId id, int index, const char *value)
	{
		// set properties of Lua entities
	}

	_declspec(dllexport) const char *__cdecl _GetProperty(EntityId id, int index)
	{
		// get properties of Lua entities
		return "";
	}
}
...
void CMonoEntityPropertyHandler::SetProperty(IEntity *entity, int index, const char *value)
{
	...
	CallMonoScript(gEnv->pMonoScriptSystem->GetEntityManager()->GetScriptId(entity->GetId()), "SetProperty", propertyInfo.name, propertyInfo.type, value);
}

const char *CMonoEntityPropertyHandler::GetProperty(IEntity *entity, int index) const
{
	return CallMonoScript(gEnv->pMonoScriptSystem->GetEntityManager()->GetScriptId(entity->GetId()), "GetProperty", m_properties.at(index).name)->Unbox<const char *>();
}