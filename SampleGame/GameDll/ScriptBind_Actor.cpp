...
float CScriptBind_Actor::GetPlayerHealth(EntityId playerId)
{
...
void CScriptBind_Actor::SetPlayerHealth(EntityId playerId, float newHealth)
{
	if(IActor *pActor = gEnv->pGameFramework->GetIActorSystem()->GetActor(playerId))
		pActor->SetHealth(newHealth);
}

float CScriptBind_Actor::GetPlayerMaxHealth(EntityId playerId)
{
	if(IActor *pActor = gEnv->pGameFramework->GetIActorSystem()->GetActor(playerId))
...
void CScriptBind_Actor::SetPlayerMaxHealth(EntityId playerId, float newMaxHealth)
{
	if(IActor *pActor = gEnv->pGameFramework->GetIActorSystem()->GetActor(playerId))
		pActor->SetMaxHealth(newMaxHealth);
}

void CScriptBind_Actor::RegisterPlayerClass(mono::string playerClass)
{
	gEnv->pGameFramework->RegisterFactory(*playerClass, (CPlayer *)0, false, (CPlayer *)0);
}