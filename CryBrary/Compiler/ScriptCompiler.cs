...
namespace CryEngine.Initialization
{
	public class ScriptCompiler
	{
		...
		public bool Initialize()
		{
			...
			compilationParams.Scripts = scripts.ToArray();
		...
		public IEnumerable<Type> CompileScripts(ref CompilationParameters compilationParameters)
		{
			...
			bool compileFromSource = compilationParameters.Sources != null && compilationParameters.Sources.Length > 0;
			if(!compileFromSource && (compilationParameters.Scripts == null || compilationParameters.Scripts.Length <= 0))
				throw new ArgumentException(message: "No scripts or source were provided.");
			...
			using(var provider = GetCodeProvider(compilationParameters.Language))
			{
				...
				{
					compilerParameters.ReferencedAssemblies.AddRange(assemblyReferenceHandler.GetRequiredAssembliesFromSource(compilationParameters.Sources));

					results = provider.CompileAssemblyFromSource(compilerParameters, compilationParameters.Sources.ToArray());
				}
				else
				{
					compilerParameters.ReferencedAssemblies.AddRange(assemblyReferenceHandler.GetRequiredAssembliesFromFiles(compilationParameters.Scripts));

					results = provider.CompileAssemblyFromFile(compilerParameters, compilationParameters.Scripts.ToArray());
				}
			}
	...
	public class CompilationParameters
	{
		...
		/// <summary>
		/// Full paths to the scripts we wish to compile. Note: Sources <b>and</b> Scripts may not be set at the same time.
		/// </summary>
		public string[] Scripts { get; set; }

		/// <summary>
		/// Source code to compile from. Note: Sources <b>and</b> Scripts may not be set at the same time.
		/// </summary>
